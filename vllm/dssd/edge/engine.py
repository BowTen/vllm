...
class EdgeDecodeEngine:
    """Edge decode orchestration for the first single-request text path."""
    ...
    def _sample_with_draft_sampler(
        ...
        if hasattr(state, "input_batch"):
            ...
            if not isinstance(hidden_states, torch.Tensor):
                raise NotImplementedError(
                    "edge draft sampling only supports tensor hidden states")

            input_batch = state.input_batch
            sample_hidden_states = cast(
                torch.Tensor, hidden_states[input_batch.logits_indices]
            )
        ...
        num_sampled = self.draft_sampler.build_num_sampled(
            input_batch.seq_lens.device
        )
