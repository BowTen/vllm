...
结果记录：`benchmarks/dssd/results/experiment-2.3-network-sensitivity-2026-04-26.md`

不限制带宽：

| 单向延迟 | Target-only token/s | DSSD token/s | DSSD Speedup | Draft Acceptance |
|---:|---:|---:|---:|---:|
| 0ms | 105.66 | 306.18 | 2.90x | 81.99% |
| 10ms | 104.77 | 149.52 | 1.43x | 81.99% |
| 20ms | 103.98 | 105.53 | 1.01x | 81.99% |
| 50ms | 101.59 | 56.73 | 0.56x | 81.99% |

100 Mbps 带宽限制：

| 单向延迟 | Target-only token/s | DSSD token/s | DSSD Speedup | Draft Acceptance |
|---:|---:|---:|---:|---:|
| 0ms | 105.64 | 285.21 | 2.70x | 81.99% |
| 10ms | 104.90 | 150.30 | 1.43x | 81.99% |
| 20ms | 103.88 | 106.59 | 1.03x | 81.99% |
| 50ms | 101.58 | 56.53 | 0.56x | 81.99% |

**预期结论**

随着链路延迟增加，DSSD 会受到更明显影响，因为它需要多轮 edge-verifier 交互。较大的 `gamma` 可以减少验证轮数，从而缓解固定往返延迟带来的损失。本次 OPT 高接受率 prompt 下，DSSD 在 0ms 和 10ms 下仍有明显收益，在 20ms 附近接近 target-only，在 50ms 时低于 target-only。100 Mbps 带宽限制对本组贪心解码实验影响较小，主要瓶颈来自固定往返延迟。
