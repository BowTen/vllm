...
class EdgeSchedulerAdapter:
    """把 DSSD edge 的一轮执行翻译成 vLLM 的 SchedulerOutput。"""
    ...
    def allocate_blocks(
        ...
        if new_blocks is None:
            raise RuntimeError("prefill 无法为 edge 申请新的 KV blocks")
    ...
    def build_decode_step(self, session: EdgeSession) -> SchedulerOutput:
        new_block_ids = self._allocate_decode_blocks(session)
        return SchedulerOutput(
            scheduled_new_reqs=[],
    ...
    def _allocate_decode_blocks(
        ...
        built_request = self._build_decode_request(session)
        num_new_tokens = built_request.num_tokens - built_request.num_computed_tokens
        new_blocks = self.kv_cache_manager.allocate_slots(
            request=built_request,
            num_new_tokens=num_new_tokens,
        ...
        if new_blocks is None:
            raise RuntimeError("decode_one 无法为 edge 申请新的 KV blocks")
        return new_blocks.get_block_ids(allow_none=True)
